...
/* SBUFFERD1 config, i.e., {<config>|none} CS203A*/
static char *cache_sbd1_opt;
...
/* StreamBuffer Data hit latency (in cycles) */
static int cache_sbi1_lat;//cs203A

static int cache_sbd1_lat;//cs203A

/* l1 instruction cache config, i.e., {<config>|dl1|dl2|none} */
static char *cache_il1_opt;

/* SBUFFERI1 config, i.e., {<config>|none} CS203A*/
static char *cache_sbi1_opt;
...
/* SBUFFER INSTRUCTION CS203A*/ 
static struct cache_t *sbufferi1 = NULL;
...
/* SBUFFER DATA CS203A*/ 
static struct cache_t *sbufferd1 = NULL;
	      ...
	      tick_t now)		/* time of access */
{
  if(sbufferd1) //CS203A
	...
	fprintf(stderr, "L1 Data - Mishandler Access");
      	return cache_access(sbufferd1, cmd, baddr, NULL, bsize,  /* now */now, /* pudata */NULL, /* repl addr */NULL); //CS203A
            // CS203A
    }        // CS203A
  else if (cache_dl2)
    {
...
/* CS203A STREAM BUFFER MISHANDLER FUNCTION ( DATA )*/
static unsigned int			/* latency of block access */
sbd1_access_fn(enum mem_cmd cmd,		/* access cmd, Read or Write */
	      md_addr_t baddr,		/* block address to access */
	      int bsize,		/* size of block to access */
	      struct cache_blk_t *blk,	/* ptr to block in upper level */
	      tick_t now)		/* time of access */
{
  fprintf(stderr,"Stream Buffer Data - Mishandler Access");
  if (cache_dl2)
    {
      /* access next level of data cache hierarchy */
      return cache_access(cache_dl2, cmd, baddr, NULL, bsize,
...
/* CS203A STREAM BUFFER MISHANDLER FUNCTION ( INSTRUCTION )*/
static unsigned int			/* latency of block access */
sbi1_access_fn(enum mem_cmd cmd,		/* access cmd, Read or Write */
	      md_addr_t baddr,		/* block address to access */
	      int bsize,		/* size of block to access */
	      struct cache_blk_t *blk,	/* ptr to block in upper level */
	      tick_t now)		/* time of access */
{
  fprintf(stderr,"Stream Buffer Instruction - Mishandler Access");
  if (cache_il2)
    {
      /* access next level of data cache hierarchy */
	      ...
	      tick_t now)		/* time of access */
{
  if(sbufferi1)  // CS203A
	...
	/* access Stream Buffer Instruction */
      	return cache_access(sbufferi1, cmd, baddr, NULL, bsize,  /* now */now, /* pudata */NULL, /* repl addr */NULL); //CS203A
            // CS203A
    }        // CS203A
  else if(cache_il2)
    {
...
"                -dtlb dtlb:128:4096:32:r\n"
  ...
  opt_reg_string(odb, "-cache:sbd1",                                 //CS203A
		 "Stream Buffer Data, i.e., {<config>|none}",
		 &cache_sbd1_opt, "sbd1:1024:8:8:f",
		 /* print */TRUE, NULL);

opt_reg_string(odb, "-cache:sbi1",                                 //CS203A
		 "Stream Buffer Instruction, i.e., {<config>|none}",
		 &cache_sbd1_opt, "sbi1:1024:8:8:f",
		 /* print */TRUE, NULL);
 ...
 opt_reg_int(odb, "-cache:sbd1lat",                               //CS203A
	      "Data Stream Buffer latency (in cycles)",
	      &cache_sbd1_lat, /* default */6,
	      /* print */TRUE, /* format */NULL);

 opt_reg_int(odb, "-cache:sbi1lat",                               //CS203A
	      "Instruction Stream Buffer latency (in cycles)",
	      &cache_sbi1_lat, /* default */6,
	      /* print */TRUE, /* format */NULL);
 ...
 if (!mystricmp(cache_sbd1_opt, "none"))
      ...
      else
	{
	  ...
	  fprintf(stderr,"assoc%d\n", assoc);
          fprintf(stderr,"cache_char2policy(c)%d\n", cache_char2policy(c));
	  fprintf(stderr,"sbd1_access_fn%d\n", sbd1_access_fn);
...
/* CS203A - CREATE SBUFFER INSTRUCTION */
 if (!mystricmp(cache_sbi1_opt, "none"))
      ...
      else
	{
	  if (sscanf(cache_sbi1_opt, "%[^:]:%d:%d:%d:%c",
		     name, &nsets, &bsize, &assoc, &c) != 5)
	    fatal("bad stream buffer parameters: "
		  "<name>:<nsets>:<bsize>:<assoc>:<repl>");
	fprintf(stderr,"Stream Buffer Instruction create");
...
sim_reg_stats(struct stat_sdb_t *sdb)   /* stats database */
{
  ...
  if(sbufferd1) //cs203a
    cache_reg_stats(sbufferd1, sdb); //cs203a
  if(sbufferi1) //cs203a
    cache_reg_stats(sbufferi1, sdb); //cs203a
  if (itlb)

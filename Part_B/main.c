...
int DVFSInterval;//cs203A
int DVFSTargetPower;//cs203A
int DVFS_flag;//cs203A
int
main(int argc, char **argv, char **envp)
{
  char *s;
  int i, exit_code;
  //cs203A For Go getting the command line values for DVFS_flag,DVFSInterval and DVFSTargetPower
  if (strcmp(argv[1],"go.alpha") == 0)
  {
   DVFS_flag = atoi(argv[5]);
   DVFSInterval = atoi(argv[6]);
   DVFSTargetPower = atoi(argv[7]);
  }//cs203A For Anagram getting the command line values for DVFS_flag,DVFSInterval and DVFSTargetPower
  if (strcmp(argv[1],"anagram.alpha") == 0)
  {
   DVFS_flag = atoi(argv[3]);
   DVFSInterval = atoi(argv[4]);
   DVFSTargetPower = atoi(argv[5]);
  }
  fprintf(stderr, "DVFSInterval: %d\n", DVFSInterval);
  fprintf(stderr, "DVFSTargetPower: %d\n", DVFSTargetPower);

 ...
 * pipeline operations.
 */
extern int DVFSTargetPower;//cs203A
extern int DVFSInterval;//cs203A
extern int DVFS_flag;//cs203A
/* simulated registers */
...
#include "machine.def"
  ...
  for (;;)
	...
	ruu_fetch_issue_delay--;
      ...
      //cs203A calling the DVFS controller every DVFSInterval cycles and checking if DVFS is on/off
      if((sim_cycle % DVFSInterval == 0) && (DVFS_flag == 1))	
	{
		fprintf(stderr,"sim_cycle %d\n",sim_cycle);
		DVFS_controller_on();
	}
      if((sim_cycle % DVFSInterval == 0) && (DVFS_flag == 0))	
	{
		fprintf(stderr,"sim_cycle %d\n",sim_cycle);
		DVFS_controller_off();
	}
